<template>
  <el-dialog v-model="visible" title="生成代码" :close-on-click-modal="false" draggable>
    <el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="120px">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="前缀" prop="dataForm.prefix">
            <el-input v-model="dataForm.prefix" placeholder="前缀" @blur="changePrefix"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
const changePrefix = () => {
  //自动生成类名
  let upperCase = dataForm.prefix.toUpperCase();
  let temp1 = dataForm.tableName.replace(upperCase,'').toLowerCase();
  let temp2 = temp1.replace(/(_\w)/g, function (match) {
    return match[1].toUpperCase();
  });
  dataForm.className = temp2.charAt(0).toUpperCase() + temp2.slice(1);
}
